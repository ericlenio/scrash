...
class SshServer extends ssh2.Server {
  static getInstance() {
    const keyPair=ssh2.utils.generateKeyPairSync('ed25519');
    console.error("ssh private key:",keyPair.private);
    const server=new this({hostKeys:[keyPair.private]});
    server.on('connection',(client,info)=>server.onConnection(client,info));
    return server;
  }

  init(sshPort) {
    return new Promise(resolve=>this.listen(sshPort,resolve));
  }

  onConnection(client,info) {
